...
- **Endurecido contra fallas en dispositivo real** (varias vueltas de depuración con pruebas del usuario en Android, ya que este entorno no tiene acceso a un dispositivo físico): la vista oculta se monta dentro de los límites reales de la pantalla en vez de fuera de ella (Android puede no renderizar vistas completamente fuera de pantalla); cada View superpuesta lleva `collapsable={false}` explícito (sin esto Android puede "aplanar" y descartar la vista del logo antes de capturarla — causa raíz confirmada, documentada oficialmente por la librería); la espera de carga del logo (recurso empaquetado en la app) ya no bloquea la exportación — solo se espera con obligación real la foto recortada (archivo nuevo cada vez); y cada paso (carga, captura, conversión final) tiene un límite de tiempo explícito con `Promise.race`/timeout, de forma que la exportación **nunca vuelve a quedarse trabada**: si algún paso no responde a tiempo, se entrega la imagen sin marca con un aviso claro en vez de congelar la pantalla.
...
- **Verificación real en dispositivo Android (Samsung S25 Ultra) por el usuario**: tras varias iteraciones de depuración remota (posicionamiento de la vista, `collapsable`, espera de carga del logo), el archivo exportado y compartido desde el dispositivo real confirmó el logo COR horneado en los píxeles correctamente. Esta es la única fase del proyecto donde la verificación final dependió de pruebas directas del usuario en su propio hardware, ya que este entorno no puede simular Android/iOS nativo.
